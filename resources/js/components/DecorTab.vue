        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3">
            <button
                ...
                :class="[
                    ...
                    activeCategory === cat.name
                        ? 'bg-violet-600 border-violet-400 text-white shadow-xl shadow-violet-500/20 scale-105 z-10'
            ...
            >
                ...
                <div :class="[
                    ...
                    activeCategory === cat.name ? 'bg-violet-700/50 border-white/20' : 'bg-slate-800 border-white/5 opacity-60'
                ]">
            ...
            <div class="relative z-10">
                <div class="flex justify-between items-end mb-4 sm:mb-6">
                    ...
                    <div class="text-right">
                        <div class="text-2xl sm:text-3xl font-black text-violet-400 font-mono">
                ...
                <div class="h-3 bg-slate-800 rounded-full overflow-hidden border border-white/5">
                    <div class="h-full bg-linear-to-r from-violet-700 via-violet-500 to-violet-400 transition-all duration-1000 relative shadow-[0_0_15px_rgba(139,92,246,0.3)]" :style="{ width: progressPercent + '%' }">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-start">
                ...
                >
                    <div class="flex justify-between items-start mb-3">
                        <span class="text-sm md:text-base font-bold text-slate-300 group-hover:text-violet-400 transition-colors">{{ translateSource(src.name) }}</span>
                        <span class="text-[10px] sm:text-xs font-mono text-slate-500">{{ src.completed }}/{{ src.total }}</span>
                    </div>
                    <div class="h-1 bg-slate-800 rounded-full overflow-hidden">
                        <div class="h-full bg-violet-600/80 transition-all duration-700" :style="{ width: (src.completed / src.total * 100) + '%' }"></div>
                    </div>
                    <div v-if="expandedSource === src.name" class="mt-4 pt-4 border-t border-white/5 space-y-1 max-h-64 overflow-y-auto no-scrollbar animate-in slide-in-from-top-2 duration-300">
                        <div v-for="item in sortedItems(src.items)" :key="item.id" class="flex items-center gap-2 text-[10px] sm:text-xs py-1">
                            ...
                            <div v-else class="w-5 h-5 rounded bg-slate-800 flex items-center justify-center text-violet-500 text-[8px] font-bold border border-white/10">D</div>
                            <a :href="item.item_id ? `https://www.wowhead.com/fr/item=${item.item_id}` : `https://www.wowhead.com/fr/search?q=${encodeURIComponent(item.name)}`" target="_blank" rel="noopener" @click.stop :class="[item.is_completed ? 'text-violet-400 font-medium' : 'text-slate-500', 'hover:underline flex-1 truncate']">{{ item.name }}</a>
        ...
        <section v-if="uncategorizedItems.length && activeCategory === 'Non classé'">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <a v-for="item in uncategorizedItems" :key="item.id" :href="item.item_id ? `https://www.wowhead.com/fr/item=${item.item_id}` : `https://www.wowhead.com/fr/search?q=${encodeURIComponent(item.name)}`" target="_blank" rel="noopener" class="flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-slate-800/40 border border-white/5 group hover:border-violet-500/30 transition-all">
                    ...
                    <div v-else class="w-10 h-10 rounded-lg bg-slate-800 flex items-center justify-center text-violet-500 font-bold border border-white/10 group-hover:scale-110 transition-transform shadow-lg shadow-violet-500/5">D</div>
...
const allDecor = computed(() => props.character.decor || []);

/** French translations for non-expansion category names (expansions stay in English) */
const CATEGORY_FR = {
    'General': 'Général',
...
/** French translations for source/subcategory names (keys kept in English in DB) */
const SOURCE_FR = {
    // Sources générales
...
/** Build category → source → items structure from flat decor array */
const categoryMap = computed(() => {
    const map = {};

    for (const item of allDecor.value) {
...
const categories = computed(() => {
    ...
    // Add "Non classé" for uncategorized items
    const uncatCount = allDecor.value.filter(i => !i.category || !i.source).length;
    if (uncatCount > 0) {
        cats.push({
            name: 'Non classé',
            total: uncatCount,
            completed: allDecor.value.filter(i => (!i.category || !i.source) && i.is_completed).length,
        });
    }
...
/** Source subcategories for the active category */
...
const uncategorizedItems = computed(() => {
    let items = allDecor.value.filter(i => !i.category || !i.source);
    if (search.value) {
        const q = search.value.toLowerCase();
        items = items.filter(i => i.name.toLowerCase().includes(q));
    }
