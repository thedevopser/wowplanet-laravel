                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-start">
                    ...
                    >
                        ...
                        <div v-if="expandedZone === zone.name" class="mt-4 pt-4 border-t border-white/5 space-y-1 max-h-48 overflow-y-auto no-scrollbar animate-in slide-in-from-top-2 duration-300">
                            <div v-for="item in sortedItems(zone.items)" :key="item.id" class="flex items-center gap-2 text-[10px] sm:text-xs py-1">
                                <a :href="`https://www.wowhead.com/fr/quest=${item.id}`" target="_blank" rel="noopener" @click.stop :class="[item.is_completed ? 'text-blue-400 font-medium' : store.isQuestCompletedElsewhere(item.id) ? 'text-amber-400/70' : 'text-slate-500', 'hover:underline flex-1 truncate']">{{ item.name }}</a>
                                <span v-if="!item.is_completed && store.getQuestOwner(item.id)" class="text-[9px] text-amber-400/60 font-mono shrink-0 truncate max-w-25">{{ store.getQuestOwner(item.id) }}</span>
                                <span v-if="item.is_completed" class="text-green-500 font-bold shrink-0">&check;</span>
                                <span v-else-if="store.isQuestCompletedElsewhere(item.id)" class="text-amber-400 shrink-0" :title="store.getQuestOwner(item.id) ? ('Fait par ' + store.getQuestOwner(item.id)) : 'Fait par un autre personnage'">
