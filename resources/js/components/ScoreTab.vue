<template>
    <div v-if="score" class="space-y-8">
        <!-- Radar + Global Score -->
            ...
            <div class="relative z-10 flex flex-col items-center gap-6">
                <div class="text-center">
                    ...
                    <p class="text-slate-500 text-xs sm:text-sm mt-1">
                        Progression globale sur {{ scoredDimensions.length }} dimension{{ scoredDimensions.length > 1 ? 's' : '' }}
        ...
        <section>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                <div
                    ...
                    :key="dim.key"
                    class="bg-slate-800/40 border border-white/5 p-4 rounded-2xl transition-colors"
                    :class="dim.applicable ? 'hover:bg-slate-800/60' : 'opacity-50'"
                >
        ...
        <!-- Low-hanging fruit: expandable categories with missing items -->
        <section v-if="recommendations.length">
            ...
            <p class="text-slate-500 text-xs sm:text-sm mb-4">Les catégories les plus proches de la complétion. Cliquez pour voir le détail de ce qu'il manque.</p>
                        ...
                        <div class="mt-3 space-y-1 max-h-64 overflow-y-auto no-scrollbar">
                            <div v-for="item in rec.missingItems" :key="item.id || item.name" class="flex items-center gap-2 text-xs sm:text-sm py-1">
                                ...
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener"
                                    class="text-slate-400 hover:text-blue-400 hover:underline truncate flex-1"
                                >{{ item.name }}</a>
                            </div>
...
const score = computed(() => store.character?.score || null);
...
function buildGroupRecs(items, type, dimension, color, groupKey) {
    ...
    for (const item of items) {
        ...
        if (item.is_completed) {
            groups[group].completed.push(item);
        } else {
            groups[group].missing.push(item);
        }
    }
    ...
    for (const [name, data] of Object.entries(groups)) {
        ...
        const missingItems = data.missing.slice(0, MAX_ITEMS_SHOWN).map(i => ({ id: i.id, name: i.name, wowheadUrl: wowheadUrl(type, i) }));
        recs.push({
            key: `${type}:${name}`,
            name,
            dimension,
...
const recommendations = computed(() => {
    ...
    // Mounts by source
    recs.push(...buildGroupRecs(store.character.mounts || [], 'mount', 'Montures', dimensionColor('mounts'), 'source'));

    // Pets by source
    recs.push(...buildGroupRecs(store.character.pets || [], 'pet', 'Mascottes', dimensionColor('pets'), 'source'));

    // Decor by source
    recs.push(...buildGroupRecs(store.character.decor || [], 'decor', 'Décorations', dimensionColor('decor'), 'source'));

    // Achievements: categories per expansion
    for (const expId in (store.character.collections || {})) {
        const categories = store.character.collections[expId]?.achievements?.categories || [];
        for (const cat of categories) {
            ...
            recs.push({
                key: `ach:${expId}:${cat.name}`,
                name: cat.name,
                dimension: 'Hauts-faits',
                color: dimensionColor('achievements'),
    ...
    // Quests: zones per expansion
    for (const expId in (store.character.collections || {})) {
        const zones = store.character.collections[expId]?.quests?.zones || [];
        for (const zone of zones) {
            ...
            if (missing.length === 0 || completed.length === 0) continue;
            recs.push({
                key: `quest:${expId}:${zone.name}`,
                name: zone.name,
                dimension: 'Quêtes',
                color: dimensionColor('quests'),
                completed: completed.length,
