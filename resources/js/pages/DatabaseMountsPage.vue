    ...
    <div class="space-y-6 py-6 sm:py-8">
        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3">
            <router-link
                ...
                :key="cat.slug"
                :to="'/base-de-donnees/montures/' + cat.slug"
                :class="[
            ...
            >
                ...
                <div :class="[
                    ...
                    activeSlug === cat.slug ? 'bg-amber-700/50 border-white/20' : 'bg-slate-800 border-white/5 opacity-60'
                ]">
                    {{ cat.count }}
                </div>
            </router-link>
            ...
            <div class="relative z-10">
                <div class="flex justify-between items-end mb-4 sm:mb-6">
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-[10px] sm:text-xs text-slate-500 font-mono uppercase font-bold">montures</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-start">
                ...
                >
                    <div class="flex justify-between items-start mb-3">
                        <span class="text-sm md:text-base font-bold text-slate-300 group-hover:text-amber-400 transition-colors">{{ src.name }}</span>
                    ...
                    <div v-if="expandedSource === src.name" class="mt-3 pt-3 border-t border-white/5 space-y-1 max-h-96 overflow-y-auto no-scrollbar animate-in slide-in-from-top-2 duration-300">
                        <div v-for="item in src.items" :key="item.id" class="flex items-center gap-3 text-xs sm:text-sm py-1.5">
                            ...
                            <a :href="item.source_spell_id ? `https://www.wowhead.com/fr/spell=${item.source_spell_id}` : `https://www.wowhead.com/fr/search?q=${encodeURIComponent(item.name_fr)}`" target="_blank" rel="noopener" @click.stop class="text-slate-400 hover:text-amber-400 hover:underline flex-1 truncate">{{ item.name_fr }}</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section v-if="uncategorizedItems.length">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <a v-for="item in uncategorizedItems" :key="item.id" :href="item.source_spell_id ? `https://www.wowhead.com/fr/spell=${item.source_spell_id}` : `https://www.wowhead.com/fr/search?q=${encodeURIComponent(item.name_fr)}`" target="_blank" rel="noopener" class="flex items-center gap-3 p-3 sm:p-4 rounded-xl bg-slate-800/40 border border-white/5 group hover:border-amber-500/30 transition-all">
                    <CollectionIcon :src="item.icon_url" :alt="item.name_fr" fallback="M" size="lg" class="text-amber-500 group-hover:scale-110 transition-transform" />
                    <div class="flex-1 min-w-0">
                        <div class="text-sm md:text-base font-bold text-slate-200 group-hover:text-amber-400 transition-colors truncate">{{ item.name_fr }}</div>
...
const breadcrumbs = computed(() => {
    ...
    if (activeSlug.value) {
        crumbs.push({ label: 'Montures', to: '/base-de-donnees/montures' });
        crumbs.push({ label: activeCategoryName.value || activeSlug.value });
    } else {
        crumbs.push({ label: 'Montures' });
    }
    return crumbs;
});
